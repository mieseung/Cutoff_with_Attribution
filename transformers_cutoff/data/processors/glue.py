...
class MnliAugProcessor(DataProcessor):
    def get_train_aug_examples(self, aug_file, aug_type):
        return self._create_examples(self._read_tsv(aug_file), 'train_aug', aug_type)

    def get_dev_aug_examples(self, aug_file, aug_type):
        return self._create_examples(self._read_tsv(aug_file), 'dev_aug', aug_type)

    def get_labels(self):
        return ['contradiction', 'entailment', 'neutral']

    def _create_examples(self, lines, set_type, aug_type):
        if aug_type == 'back_trans':
            guid_idx, text_a_idx, text_b_idx, label_idx = 0, 3, 5, 6
        elif aug_type == 'cbert':
            guid_idx, text_a_idx, text_b_idx, label_idx = 0, 1, 2, 3
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            guid = '%s-%s' % (set_type, line[guid_idx])
